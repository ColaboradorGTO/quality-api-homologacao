...
class PDFGNRE {
  ...
  static async desenharVia(doc, { x, y, width, height, tituloVia, nfe, calculo, dadosGNRE, numeroControle, linhaDigitavel }) {
    ...
    doc.font('Helvetica').text(ufEmitente, rightColX, rowStartY + rowGap * 2 + 8, { width: rightColW, align: 'left', ellipsis: true });

    doc.font('Helvetica-Bold').text('Telefone:', rightColX, rowStartY + rowGap * 3, { width: rightColW, ellipsis: true });
    doc.font('Helvetica').text(telefoneEmitente, rightColX, rowStartY + rowGap * 3 + 8, { width: rightColW, align: 'left', ellipsis: true });
    ...
    doc.font('Helvetica').fontSize(6).text('Informações Complementares:', x + 4, validY + 5);
    doc.text(infoComplementar, x + 120, validY + 5, { width: mainWidth - 125, ellipsis: true });
