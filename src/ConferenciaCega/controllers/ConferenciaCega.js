...
class ConferenciaCegaControllers  {
    ...
    async putStatusDivergencia(req, res) {
        try {
            ...
            const response = await axios.put(`${url}/api/conferencia-cega/status-divergencia.xsjs`, dados);
    ...
    async postStatusDivergencia(req, res) {
        try {
            const dados = Array.isArray(req.body) ? req.body : [req.body]; 
            const response = await axios.post(`${url}/api/conferencia-cega/status-divergencia.xsjs`, dados);
            return res.json(response);
        } catch (error) {
            console.error("Unable to connect to the database:", error);
            return res.status(500).json({ error: error.message });
